...
async function handleRequest(fetchPath) {
  if (!fetchPath.includes("among-us-online")) {
    ...
    try {
      ...
      var htmlCode1 = await customFetch.text();

      var htmlAddon1 = await fetch("/addon.html");
      var htmlAddon = await htmlAddon1.text();
      
      var htmlCode = htmlCode1 + htmlAddon;
    ...
    try {
      return await fetch("https://amongus.mixxer.repl.co/index.html");
    } catch (error) {
      console.error(`Fetch request for ${fetchPath} failed with error: ${error}`);
    }
  }
...
self.addEventListener("fetch", function (e) {
  ...
  if (path == "/files/among-us-online/index.html") {
    return e.respondWith(handleRequest("among-us-online"));
